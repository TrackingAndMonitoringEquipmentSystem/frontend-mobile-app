...
import 'package:frontend/features/manage_locker_and_equipment/presentation/widgets/confirm_delete_permission_popup.dart';

class PermanentPermissionWidget extends HookWidget {
  const PermanentPermissionWidget({Key? key}) : super(key: key);
            ...
            onSelected: (value) {
              if (value == 1) {
                AutoRouter.of(context)
                    .push(ChangePermissionTypeRoute(isPermanentType: true));
              } else {
                showDialog(
                  context: context,
                  builder: (context) => const ConfirmDeletePermissionPopup(
                    isPermanentType: true,
                  ),
                );
              }
            },
